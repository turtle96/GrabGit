	...
	d3.json(url, function(error, data) {
		...
		data = data.filter(function(author) {
			...
			names.forEach(function(name) {
				if (name.toLowerCase() == author[0].name.toLowerCase()) {
					result = true;
				}
			}); 
		...
		path.on("click", function(d) {
		    ...
		    commitDisplay.selectAll("div")
		    	...
		    	.html(function(d) { 
		    		...
		    		commit += "<font color=#689F38>Added " + d.add + "</font> lines<br>";
		    		commit += "<font color=#D32F2F>Deleted " + d.delete + "</font> lines<br>";
		    		return commit; 
		    	});
		});
	});
